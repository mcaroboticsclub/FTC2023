...
@Autonomous(name="Auton left cycling", group="Exercises")
//@Disabled
public class autonLeftCycling extends LinearOpMode
{
    ...
    //rot=(inches/(wheelDiameter*(Math.PI)))*537.6
    //inches=rot*(wheelDiameter*(Math.PI)))/537.6;
    double convertToInches(double rot) { return rot*(wheelDiameter*(Math.PI))/537.6; }
    ...
    double radianToDegree=57.296;
    //14.25
    double chassisDiam=14.25;
    public void rotate(int angle){
        //(encR-encL)*radianToDegree/chassisDiam
        double encL=convertToInches(frontLeft.getCurrentPosition());
        double encR=convertToInches(frontRight.getCurrentPosition());
        double curAngle=(encR-encL)*radianToDegree/chassisDiam;
